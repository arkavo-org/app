...
class MockArkavoClient: @unchecked Sendable {
    ...
    func encryptAndSendPayload(payload: Data, policyData: Data) async throws {
...
class MockKeyStoreForExchange {
    ...
    func serialize() async -> Data {
        "serializedKeyStoreData".data(using: .utf8)!
    }

    func exportPublicKeyStore() async -> MockPublicKeyStoreForExchange {
        MockPublicKeyStoreForExchange()
    }
}
...
class MockPublicKeyStoreForExchange {
    ...
    func serialize() async -> Data {
        "serializedPublicKeyStoreData".data(using: .utf8)!
    }
}
