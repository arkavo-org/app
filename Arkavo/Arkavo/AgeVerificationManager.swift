...
//    }
    ...
    func promptForSelfie(cardData _: IDCardData) {
        // Implement selfie capture UI
        // This would be similar to the ID card scanner but optimized for face capture
        // For now, we'll just use a placeholder
        print("Implement selfie capture UI")
    }

    func processVerification(idCardImage: UIImage, selfieImage: UIImage) async {
        do {
            // Process ID card
            let cardData = try await idVerificationManager.processIDCard(image: idCardImage)

            // Verify face match
            let faceMatches = try await idVerificationManager.compareFaces(
                idCardImage: idCardImage,
                selfieImage: selfieImage
            )

            // Verify age from extracted date of birth
            if let dateOfBirth = cardData.dateOfBirth,
