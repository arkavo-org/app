...
class StreamViewModel: ObservableObject {
    ...
    func refreshStreamData(context: ModelContext) {
        ...
        if let updatedStream = try? context.fetch(descriptor).first {
            ...
            print("StreamViewModel: Refreshed stream data for \(updatedStream.streamName)")
        }
    }
}
...
struct CompactStreamProfileView: View {
    ...
    var body: some View {
        HStack {
            Text(viewModel.stream?.streamName ?? "Unknown")
                .font(.headline)
            Spacer()
        }
        .padding(.vertical, 8)
...
struct DetailedStreamProfileView: View {
    @StateObject var viewModel: StreamViewModel
    @EnvironmentObject var peerDiscoveryManager: PeerDiscoveryManager
                ...
                if let stream = viewModel.stream {
                    Form {
                        Section(header: Text("Profile")) {
                            ...
                            if !stream.streamBlurb.isEmpty {
                                Text("\(stream.streamBlurb)")
                            }
                        }
...
struct CreateStreamProfileView: View {
    ...
    var body: some View {
        ...
        .onChange(of: viewModel.blurb) { _, _ in viewModel.validateBlurb() }
        .onAppear {
