...
/// Diagnostic tests for ARKitCoordinateConverter
///
...
final class ARKitCoordinateConverterTests: XCTestCase {
    ...
    func testIdentityMatricesProduceTpose() {
        ...
        let originalCalibration = ARKitCoordinateConverter.restPoseCalibrationEnabled
        ARKitCoordinateConverter.restPoseCalibrationEnabled = false
        defer { ARKitCoordinateConverter.restPoseCalibrationEnabled = originalCalibration }
        ...
        if let hipsTransform = skeleton.joints[.hips] {
            let hipsRotation = ARKitCoordinateConverter.computeVRMRotation(
                joint: .hips,
        ...
        for joint in [ARKitJoint.spine, .chest, .upperChest, .neck, .head] {
            guard let transform = skeleton.joints[joint] else { continue }
            if let rot = ARKitCoordinateConverter.computeVRMRotation(
                joint: joint,
                childTransform: transform,
                skeleton: skeleton
            ) {
    ...
    func testRootRotationCorrectionAnalysis() {
        let correction = ARKitCoordinateConverter.rootRotationCorrection
    ...
    func testHipsYawRotation() {
        ...
        if let result = ARKitCoordinateConverter.computeVRMRotation(
            joint: .hips,
    ...
    func testArmRaiseDirection() {
        ...
        guard let resultZ = ARKitCoordinateConverter.computeVRMRotation(
            joint: .leftUpperArm,
        ...
        guard let resultY = ARKitCoordinateConverter.computeVRMRotation(
            joint: .leftUpperArm,
    ...
    func testLeftSideMirroring() {
        ...
        // Apply to left arm
        let leftResult = ARKitCoordinateConverter.convertLocalRotation(testRotation, joint: .leftUpperArm)
        // Apply to right arm
        let rightResult = ARKitCoordinateConverter.convertLocalRotation(testRotation, joint: .rightUpperArm)
    ...
    func testConversionPipelineDetailedAnalysis() {
        ...
        let directResult = ARKitCoordinateConverter.convertLocalRotation(testRotation, joint: .leftUpperArm)
    ...
    func testExtractRotationWithScale() {
        ...
        let extracted = ARKitCoordinateConverter.extractRotation(from: matrix)
    ...
    func testParentHierarchyCompleteness() {
        ...
        let parentMap = ARKitCoordinateConverter.arkitParentMap
    ...
    func testFullPipelineWithRealisticData() {
        ...
        for joint in keyJoints {
            guard let transform = skeleton.joints[joint] else { continue }
            if let rotation = ARKitCoordinateConverter.computeVRMRotation(
                joint: joint,
                childTransform: transform,
                skeleton: skeleton
            ) {
